...
function getMetaByNodeDir(linkPath) {
  let filePath = linkPath.replace(getRootDir(), '')
  if (filePath.startsWith('/')) {
    filePath = filePath.slice(1)
  }
  const [type, versionAndArch] = filePath.split('/')
  const { version, arch } = extractVersionAndArch(versionAndArch)
  return { isChakraCore: type === 'chakracore', version, arch }
}
...
function getNodeDirByMeta(version, isChakraCore, arch) {
  ...
  arch = getArch(arch)
  const dir = isChakraCore ? 'chakracore' : 'node'
  return getRootDir(dir, `${version}-${arch}`)
}
