...
async function getMirrorUrlByTypeAndVersion(version, isChakraCore, arch) {
  version = version.toLowerCase()
  let mirrorType = 'node'
  let mirror

  if (version === 'lts') {
    mirror = getMirrorUrlByType(mirrorType, isChakraCore)
...
async function add(inputVersion, isChakraCore, arch, force = false) {
  ...
  const {
    ...
    tempFile,
    destDir,
  } = await getMirrorUrlByTypeAndVersion(inputVersion, isChakraCore, arch)
